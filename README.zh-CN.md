...
**every-claude-code-learning** 本仓库以**教大家怎样使用 Everything Claude Code（ECC）**为目标：ECC 是面向 Claude Code、Codex、Cowork 等 **AI 编程 harness** 的生产级插件体系（agents、skills、斜杠命令、hooks、rules、MCP）。你在本仓库中可以照教程动手、运行测试脚本并对照真实目录结构练习。

| 你想… | 从这里开始 |
|--------|------------|
| 按章节学习与实验（推荐） | [**简体中文教程（8 篇）**](docs/zh-CN/tutorial/README.md) |
